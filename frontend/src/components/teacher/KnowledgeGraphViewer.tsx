...
const ConceptNode = ({ data }: { data: KGNodeData }) => (
    <div
        className={`px-3 py-2 rounded-lg border-2 text-xs max-w-[160px] shadow-sm text-center
...
export default function KnowledgeGraphViewer() {
    ...
    useEffect(() => {
        fetch('http://localhost:8000/api/teacher/kg_nodes')
            ...
            .then((data) => {
                ...
                // Layout: 5 columns, rows spaced 140px apart
                const kgNodes: Node[] = rawNodes.map((n, i) => ({
                    id: n.node_id,
                    type: 'conceptNode',
                    position: {
                        x: (i % 5) * 220 + 40,
                ...
                // Sequential edges between adjacent nodes (concept chain)
                const kgEdges: Edge[] = kgNodes.slice(0, -1).map((_, i) => ({
                    id: `e-${i}-${i + 1}`,
                    source: kgNodes[i].id,
    ...
    if (isLoading) {
        return (
            <div className="flex items-center justify-center h-64 text-sm text-gray-400">
                ...
                Loading Knowledge Graph from MongoDB...
            </div>
        );
    }
    ...
    if (error) {
        return (
            <div className="flex items-center justify-center h-32 text-sm text-red-500 bg-red-50 rounded-lg border border-red-200">
                ❌ Failed to load KG: {error}. Is the FastAPI server running?
            </div>
        );
    }
    ...
    if (nodes.length === 0) {
        return (
            <div className="flex items-center justify-center h-32 text-sm text-gray-400 italic bg-slate-50 rounded-lg border border-slate-200">
                No Knowledge Graph data found. Upload a syllabus above to populate it.
            </div>
        );
    }
    ...
    return (
        <div>
            ...
            <div className="flex items-center gap-3 mb-3">
                ...
                <span className="text-xs text-gray-400">Extracted from ingested syllabus · Stored in MongoDB</span>
            </div>
